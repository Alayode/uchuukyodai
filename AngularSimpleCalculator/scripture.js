...
function Seigyoki($scope) {
    ...
    // Constants
    var MULTIPLICATION = "multiplying";
    var MULTIPLICATION_TOKEN = "*";
    var ADD = "adding";
     ...
     * newNumber flag
     */
    $scope.updateOutput = function (button) {
        if ($scope.output == "0" || $scope.newNumber) {
            $scope.output = button;
            $scope.newNumber = false;
        } else {
            $scope.output += String(button);
        }
    ...
    $scope.multiply = function () {
        if ($scope.pendingValue) {
            if ($scope.runningTotal && $scope.pendingOperation == MULTIPLICATION) {
                $scope.runningTotal *= $scope.pendingValue;
            } else if ($scope.runningTotal && $scope.pendingOperation == MULTIPLICATION) {
                $scope.runningTotal *= $scope.pendingValue;
            } else {
                $scope.runningTotal = $scope.pendingValue;
            }
        }
        setOperationToken(MULTIPLICATION);
        setOutput(String($scope.runningTotal));
        $scope.pendingOperation = MULTIPLICATION;
        $scope.newNumber = true;
        $scope.pendingValue = null;
    };
    ...
    $scope.calculate = function () {
        ...
        } else if ($scope.pendingOperation == MULTIPLICATION) {
            $scope.runningTotal *= $scope.pendingValue;
